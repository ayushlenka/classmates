...
export default function Input() {
  ...
  const handleSubmit = async (e) => {
    ...
    setTimeout(() => {
      navigate('/transcripts/123');
    }, 1000);
  };

  return (
    <div className="px-24">
      <h1 className="mt-32 mb-20 text-white text-4xl font-thin font-archivo">
        Schedule Generator
      </h1>

      <form onSubmit={handleSubmit} className="flex flex-col space-y-12 max-w-4xl">
        {/* File Upload */}
        <div className="flex flex-col">
          <label className="mb-3 text-white text-lg font-thin font-archivo">
        ...
        <div className="flex gap-5">
          <div className="flex flex-col w-1/2">
            ...
            <select
              ...
              className="p-5 rounded-md border border-gray-500 bg-[#282828] text-white"
              required
            >
        ...
        <button
          type="submit"
          disabled={loading}
          className={`mt-5 w-44 py-5 text-white text-lg font-archivo rounded-full bg-[rgba(188,85,236,0.35)] shadow-lg ${loading ? 'opacity-50' : 'opacity-100'}`}
        >
          {loading ? (
